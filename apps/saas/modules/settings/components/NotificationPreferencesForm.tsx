...
} from "@repo/notifications/catalog";
...
export function NotificationPreferencesForm() {
	...
	const { data, isPending } = useQuery(orpc.notifications.getPreferences.queryOptions());
		...
		<Card className="p-6">
			{isPending ? (
				...
				<div className="gap-8 flex flex-col">
					{NOTIFICATION_GROUPS.map((group: NotificationGroupConfig) => (
						<section key={group.id}>
							...
							<div className="overflow-x-auto rounded-lg border">
								<table className="text-sm w-full min-w-[320px]">
									...
									<tbody>
										{group.types.map((type: NotificationTypeId) => (
