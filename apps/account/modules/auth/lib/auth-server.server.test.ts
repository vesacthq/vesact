...
import { getOrganizationBySlug, getOrganizationList } from "./auth-server.server";
...
describe("getOrganizationBySlug", () => {
	it.each([
		...
		await expect(getOrganizationBySlug("acme")).resolves.toBeNull();
	});
	...
	it("propagates an expired session instead of pretending there is no organization", async () => {
		...
		await expect(getOrganizationBySlug("acme")).rejects.toBeInstanceOf(APIError);
	});
	...
	it("propagates a failure and answers again once the backend recovers", async () => {
		...
		await expect(getOrganizationBySlug("acme")).rejects.toThrow("connection reset");
		await expect(getOrganizationBySlug("acme")).resolves.toEqual(organization);
	});
});
