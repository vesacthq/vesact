...
`apps/studio/.dev.vars` (copy it from `apps/studio/.dev.vars.example`) is what the
...
`VITE_STUDIO_URL` and `VITE_MARKETING_URL` are the public URLs the apps advertise:
...
- Six seams are unavoidable and conflict on any merge that touches them:
  ...
  `apps/studio/modules/shared/components/AppSidebar.tsx`,
  `packages/permissions/definition.ts`, `packages/database/drizzle/client.ts`,
  and `apps/studio/server.ts`. The last two carry the Cloudflare Workers
