...
幂等（§5.4.3 的语义）由 `lib/idempotency.ts` 的 `idempotent` 中间件实现，挂在具体的 POST 路由上、`relayKey` 之后；A1 没有 POST，A3 的发送端点第一个挂。摘要是原始 body 的 SHA-256，作用域是 `apiKeyId` + 请求路径（含路径参数，不含 query）。争用靠一条 upsert：插入 `in_flight` 行，或接管 `expiresAt` 已过的行；`in_flight` 行的 `expiresAt` 是 60 秒锁，60 秒内没完成就能被接手，完成后改成 24 小时并存状态码和 body；存响应失败只记日志，响应照常返回，锁到期自然放开。没抢到就读行：`in_flight` → 409，摘要不同 → 422，否则回放并带 `Idempotent-Replayed: true`。oRPC 把 handler 的异常转成响应后才回到中间件，所以 5xx 也会存下来。
