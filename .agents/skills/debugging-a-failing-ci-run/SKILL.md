...
1. Identify the failing run and job:
   ...
   ```
2. Map the job to its exact command: `pnpm verify`, `pnpm type-check`, `pnpm build`, `pnpm test`, or the sequential marketing/Studio `e2e:ci` commands. `pnpm verify` is marketing generation, Oxlint, then `oxfmt --check`; `pnpm lint` is not equivalent.
...
5. Reduce to a focused command after reproducing, such as:
   ...
   ```
6. For Playwright, note that CI uploads only `apps/studio/playwright-report/` as `playwright-report`; inspect job logs for a marketing failure because its report path is not included in that artifact.
...
Canonical reference: `.github/workflows/validate-prs.yml`; `apps/studio/playwright.config.ts` documents server reuse and CI retry behavior.
...
- Looking for a marketing HTML report in the uploaded artifact when the workflow only includes Studio.
