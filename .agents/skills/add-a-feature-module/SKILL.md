...
description: "Use when introducing a cohesive Studio, account center or marketing feature module with TanStack Router routes, Form state, Query state, and local aliases."
---
...
1. Choose the owning app and create `apps/studio/modules/<feature>/`, `apps/account/modules/<feature>/` or `apps/marketing/modules/<feature>/` with only needed `components/`, `hooks/`, and `lib/` directories.
...
5. Add navigation in the owning shell when required, such as `apps/studio/modules/shared/hooks/use-app-nav.ts`, and translate every label. When adding a top-level route under `apps/studio/routes/_authenticated/_main/`, append its slug to `organizations.forbiddenOrganizationSlugs` in `packages/auth/config.ts` so it cannot collide with an organization slug.
...
7. Let the TanStack Router Vite plugin regenerate `routeTree.gen.ts` during dev/build; never edit it. Run the owning app:
   ...
   # Or replace `studio` with `account` or `marketing`.
   ```
8. Run focused tests plus `pnpm format`, `pnpm lint`, and `pnpm type-check`.

Canonical references: `apps/account/modules/payments/` with `apps/account/routes/_authenticated/_settings/orgs/$organizationSlug/billing/index.tsx`; `apps/marketing/modules/blog/` with `apps/marketing/routes/blog/`; and each app's `tsconfig.json`.
...
- Adding a top-level Studio route without reserving its slug from organization URLs.
