...
4. Add `settings.notificationsPage.types.<TYPE>.label` in every `packages/i18n/translations/{en,zh}/account.json`. If the type appears in `NOTIFICATION_GROUPS`, extend the explicit `onToggle` type in `apps/account/modules/account/components/NotificationPreferencesForm.tsx` to include the new `NotificationTypeId`.
...
6. Dispatch with `createNotification({ userId, type, data: { title, message }, link })`. It resolves relative links against `VITE_STUDIO_URL`, checks `IN_APP` and `EMAIL` disable rows independently, persists the in-app row when enabled, and sends the generic React Email template when enabled.
...
8. Test both channel suppression paths and trigger behavior; run `pnpm --filter @repo/database generate`, focused tests, lint, and type-check. Never hand-edit the generated migration.
