...
4. Add `settings.notificationsPage.types.<TYPE>.label` in every `packages/i18n/translations/{en,de,es,fr}/studio.json`. If the type appears in `NOTIFICATION_GROUPS`, extend the explicit `onToggle` type in `apps/studio/modules/settings/components/NotificationPreferencesForm.tsx` to include the new `NotificationTypeId`.
