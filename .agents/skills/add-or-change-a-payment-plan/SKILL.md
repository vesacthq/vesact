...
4. Add plan title, description, and features under `pricing.products.<planId>` to all locales in `packages/i18n/translations/{en,zh}/shared.json`.
5. Marketing `PricingSection` and `usePlanData()` in the account center and Studio iterate config keys dynamically, so they need no per-plan code, only the matching `pricing.products.<planId>` messages.
...
Canonical references: `packages/payments/config.ts`, `packages/payments/lib/provider-price-ids.ts`, `apps/marketing/modules/home/components/PricingSection.tsx`, `apps/account/modules/payments/hooks/plan-data.tsx`, and `packages/api/modules/payments/procedures/create-checkout-link.ts`.
