...
4. Add plan title, description, and features to all locales in `packages/i18n/translations/{en,de,es,fr}/marketing.json` and `saas.json`.
5. Update the marketing page's explicit `productTitle`/`productDescription`/`productFeatures` switches. SaaS `usePlanData()` iterates config keys dynamically but still requires matching `pricing.products.<planId>` messages.
...
Canonical references: `packages/payments/config.ts`, `packages/payments/lib/provider-price-ids.ts`, `apps/marketing/modules/home/components/PricingSection.tsx`, `apps/saas/modules/payments/hooks/plan-data.tsx`, and `packages/api/modules/payments/procedures/create-checkout-link.ts`.
...
- Adding a plan to dynamic SaaS data but omitting the marketing switch cases.
