...
- **Production dependencies**: Bumped `ai` to `^7.0.50`, `@ai-sdk/anthropic` to `^4.0.28`, `@ai-sdk/openai` to `^4.0.28`, `@ai-sdk/react` to `^4.0.53`, `@orpc/*` to `1.14.14`, `@tanstack/react-start` to `^1.168.35`, `nanoid` to `^6.0.1`, `@aws-sdk/client-s3` and `@aws-sdk/s3-request-presigner` to `3.1102.0`, `hono` to `^4.13.0`, and `openai` to `^7.4.0`. Skipped `@types/uuid` (deprecated). Refresh the lockfile with `pnpm install` after pulling. `pnpm-workspace.yaml` enforces `minimumReleaseAge: 1440` (one day) at install time.
