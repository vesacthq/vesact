...
- **App shell**: The SaaS shell is rebuilt on the official `Sidebar` following the shadcn `sidebar-07` composition for the `base-vega` style, with the ReUI `app-shell-1` block as the structural reference only (none of its class overrides are kept, so spacing, button sizes, and colors come from the vega component and the theme tokens). Icon-collapsible rail with `SidebarRail` (state persisted in the `sidebar_state` cookie and read in the `_main` route loader), the organization switcher as the sidebar header (`size="lg"` trigger with logo, name, and plan), collapsible nav groups (inline submenu when expanded, side dropdown when collapsed), the user menu in the footer (account links, color mode toggle, logout), and a header with `SidebarTrigger`, a breadcrumb derived from the active nav item, and the notification center. The old `NavBar`, `sidebar-context`, and `use-media-query` are removed; `OrganizationSelect` (renamed from `OrganzationSelect`) and `UserMenu` no longer take props.
...
- **Production dependencies**: Bumped `ai` to `^7.0.50`, `@ai-sdk/anthropic` to `^4.0.28`, `@ai-sdk/openai` to `^4.0.28`, `@ai-sdk/react` to `^4.0.53`, `@orpc/*` to `1.14.14`, `@tanstack/react-start` to `^1.168.35`, `nanoid` to `^6.0.1`, `@aws-sdk/client-s3` and `@aws-sdk/s3-request-presigner` to `3.1102.0`, `hono` to `^4.13.0`, and `openai` to `^7.4.0`. Removed deprecated `@types/uuid` stub (the `uuid` package ships its own TypeScript definitions). Refresh the lockfile with `pnpm install` after pulling. `pnpm-workspace.yaml` enforces `minimumReleaseAge: 1440` (one day) at install time.
