...
in dev and preview and run their Docker target (below) in production (marketing also as a Worker on `www.allcast.ai`, the overseas site), on one machine in
...
| Worker        | —                                          | `vesact-<app>-preview`                                                                                                                         | `vesact-relay`, `vesact-www`, `vesact-marketing` (`www.allcast.ai`)                                                       |
| Host          | `localhost:300x`                           | `app.preview.allcast.ai` (account at `/account`), `www.preview.allcast.ai`; Vesact `www.`, `console.`, `api.preview.vesact.com`; behind Access | `app.allcast.cc` (account at `/account`), `www.allcast.cc`, `www.allcast.ai`; Vesact `www.`, `console.`, `api.vesact.com` |
| Vars          | `.dev.vars`                                | `env.preview.vars` in `wrangler.jsonc`                                                                                                         | `secrets/<app>.prod.env` → `env/<app>.env` on the machine (Relay and the two website Workers: top-level `vars`)           |
...
own, the `www` job having no secrets); on `main` the relay, `www` and marketing jobs deploy their
production Workers the same way (marketing's is the overseas `www.allcast.ai`), and `images` + `machine`
